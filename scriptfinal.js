...
const priceScreen = document.getElementById('display-total');
...
if (totalCID < changeDue) {
  displayChangeDue.innerHTML = `Status: INSUFFICIENT_FUNDS</p>`;
  return;
}
...
const updateUI = (change) => {
  ...
  cashDrawerDisplay.innerHTML = `<p>Change in drawer:</p>
                                  ${cid.map(([denominationName, amount]) => `<p>${denominationNames[denominationName]}: \$${amount}</p>`).join("")}`
}
